...
async function extractVerb(verb, cutBase = "") {
  ...
  result["passé simple"] = extractTense(
    doc,
    ".indicative .simple-past",
    cutBase,
  );
  result["futur simple"] = extractTense(doc, ".indicative .future", cutBase);
  result["subjonctif présent"] = extractTense(
    doc,
    ".subjunctive .present",
    cutBase,
  );
  result["subjonctif imparfait"] = extractTense(
    doc,
    ".subjunctive .imperfect",
    cutBase,
  );
  result["conditionnel présent"] = extractTense(
    doc,
    ".conditional .present",
    cutBase,
  );
  result["imperatif"] = extractTense(doc, ".imperative .present", cutBase);
  result["participe présent"] = extractTense(
    doc,
    ".participle .present",
    cutBase,
  );
  result["participe passé"] = extractTense(
    doc,
    ".participle .past",
    cutBase,
  ).slice(0, 1);
...
async function extractVerbs() {
  ...
  fs.writeFileSync(
    `${SRC}/fr/irregular_verbs.json`,
    JSON.stringify(irregularVerbsTenses),
  );
  ...
  fs.writeFileSync(
    `${SRC}/fr/regular_verbs.json`,
    JSON.stringify(regularVerbsEndings),
  );
}
